...
export const getAllProducts = async (req: Request<IdParams>, res: Response) => {
  try {
...
export const updateProduct = async (req: Request<IdParams>, res: Response) => {
  try {
    const { userId } = getAuth(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
...
export const deleteProduct = async (req: Request<IdParams>, res: Response) => {
  try {
    ...
    // Check if product exists and belongs to user
    const existingProduct = await queries.getProductById(userId);
    if (!existingProduct) {
      res.status(404).json({ error: "Product not found" });
      return;
    }
